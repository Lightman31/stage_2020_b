...
<body>
	...
	include ("connect.php");
		...
		$req = $bdd->query('SELECT Statut.nom_statut, Statut.inf_dat FROM SAV, Statut WHERE SAV.num_sav = "'.$_POST['num_sav1'].'" AND SAV.id_sav = Statut.sav ORDER BY Statut.inf_dat DESC;'); 
		...
		{
			echo '<tr><td>';
			echo $donnees['inf_dat'].' : </td><td>'. $donnees['nom_statut'];
			echo '</td></tr>';
		}
		echo '</table>';
	?>
