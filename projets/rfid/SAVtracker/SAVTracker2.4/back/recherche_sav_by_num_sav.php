...
<?php
		...
		<div class ="col-md-6">
			<?php  
				...
				if ($line_data = $data->fetch())
				{
					...
					echo '</ h3>';
					echo '<h3>';
					echo $line_data['delai_theorique'];
					echo '</ h3>';
				}
			?>
		</div>
	...
	<tbody>
		...
		$reponse = $bdd->query('SELECT Emplacement.nom_emplacement,Emplacement.inf_dat_emplacement FROM Emplacement WHERE Emplacement.sav = (SELECT id_sav FROM SAV WHERE num_sav="'.$_POST['num_sav1'].'") ORDER BY inf_dat_emplacement DESC;');
/*
...
*/
	...
	<tbody>
		...
		//include ("connect.php");
		//$bdd = connectBDD();
		$statut = $bdd->query('SELECT Statut.nom_statut,Statut.inf_dat_statut FROM Statut WHERE Statut.sav = (SELECT id_sav FROM SAV WHERE num_sav="'.$_POST['num_sav1'].'") ORDER BY inf_dat_statut DESC;');
/*
		$query_statut = $bdd->query(' SELECT * FROM Statut where inf_dat_statut IN (SELECT MAX(inf_dat_statut) FROM Statut GROUP by sav) GROUP BY sav ;'); 
